...
public class Cell implements ReportCell {
     ...
     * 当前单元格所在行所有子格
     */
    private final Map<String, List<Cell>> rowChildrenCellsMap = new HashMap<String, List<Cell>>();
    /**
     * 当前单元格所在列所有子格
     */
    private final Map<String, List<Cell>> columnChildrenCellsMap = new HashMap<String, List<Cell>>();
    private String name;
    private int rowSpan;
    private int colSpan;
    /**
    ...
    public Object getFormatData() {
        if (formatData == null) {
            // return data;
            return doFormatData(data);
        }
        // return formatData;
        return doFormatData(formatData);
    }
    ...
    /**
     * 20200516  chenhk  先反编码一下  因为我们从数据库拿出来的数据是已经编码的
     * @param data
     * @return
     */
    private Object doFormatData(Object data) {
